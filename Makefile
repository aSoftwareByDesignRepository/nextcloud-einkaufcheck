# Makefile for EinkaufCheck (einkaufcheck) app release

app_name = einkaufcheck
build_dir = build
...
release:
	...
	@staging=$$(mktemp -d) && \
		...
		rsync -a --exclude='.git' --exclude='$(build_dir)' --exclude='.github' \
			...
			--exclude='.auth' --exclude='.phpunit.result.cache' --exclude='test-results' \
			...
			--exclude='openapi.json' --exclude='__pycache__' --exclude='appinfo/signature.json' \
			./ "$$staging/$(app_name)/" && \
...
verify-release:
	...
	@if tar -tzf $(archive_path) | grep -Eq '/(\.git/|node_modules/|build/|tests/|e2e/|\.auth/|test-results/|scripts/|vendor/)'; then \
		echo "Error: release archive contains forbidden development paths"; \
		tar -tzf $(archive_path) | grep -E '/(\.git/|node_modules/|build/|tests/|e2e/|\.auth/|test-results/|scripts/|vendor/)' || true; \
		exit 1; \
	fi
	@test $$(tar -tzf $(archive_path) | grep -c '^einkaufcheck/python/') -gt 0 || (echo "Error: python runtime missing from archive"; exit 1)
...
verify-signature-manifest:
	...
	@tmpdir=$$(mktemp -d) && \
		...
		if grep -Eq '"([^"]*/)?(\.git|node_modules|build|tests|e2e|\.auth|test-results|scripts|vendor)\\/' "$$sig"; then \
			echo "Error: signature.json references forbidden development paths"; \
			grep -E '"([^"]*/)?(\.git|node_modules|build|tests|e2e|\.auth|test-results|scripts|vendor)\\/' "$$sig" || true; \
			exit 1; \
		fi
